...
const ServiceCard: React.FC<ServiceCardProps> = ({ service, onClick, language }) => {
  ...
  return (
    <div 
      className="group relative flex flex-col p-8 bg-white border border-gray-100 shadow-sm hover:shadow-2xl transition-all duration-500 cursor-pointer overflow-hidden text-start"
    ...
    >
      ...
      <div className="flex items-center gap-3 text-xs font-black uppercase tracking-widest text-[#0a0b0d] group-hover:gap-5 transition-all">
        {t.explore}
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className={`w-4 h-4 text-[#0037f3] ${language === 'ar' ? 'rotate-180' : ''}`}>
